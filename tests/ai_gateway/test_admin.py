...
class TestAIGatewayTeamAdminChangeView:
    ...
    def test_rendering_fetches_from_gateway_once(self, client, superuser, team, key_service):
        client.force_login(superuser)
        url = reverse("admin:ai_gateway_team_change", args=[team.pk])

        client.get(url)

        assert key_service.list_access_groups.call_count == 1
        assert key_service.get_team_access_group_ids.call_count == 1

    def test_viewing_logs_current_access_groups(
        self, client, superuser, team, key_service, caplog
    ):
        client.force_login(superuser)
        url = reverse("admin:ai_gateway_team_change", args=[team.pk])
    ...
    def test_saving_updates_team_access_groups(self, client, superuser, team, key_service):
        client.force_login(superuser)
        url = reverse("admin:ai_gateway_team_change", args=[team.pk])
    ...
    def test_load_error_propagates(self, client, superuser, team, key_service):
        key_service.list_access_groups.side_effect = AIGatewayAPIError(503, "gateway down")
        client.force_login(superuser)
        url = reverse("admin:ai_gateway_team_change", args=[team.pk])
